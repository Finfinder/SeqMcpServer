...
- `.github/workflows/reusable-open-next-version-branch.yml` — added `source-repository` and `workflow-run-id` inputs so the reusable workflow can propagate caller identity and run correlation metadata to downstream version-branch creation steps
